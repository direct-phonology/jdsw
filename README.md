...
|儀禮|[_Etiquette and Ceremonial_](https://en.wikipedia.org/wiki/Etiquette_and_Ceremonial)|[KR1d0026](https://github.com/kanripo/KR1d0026)*|10|
...
|春秋公羊傳|[_Commentary of Gongyang_](https://en.wikipedia.org/wiki/Gongyang_Zhuan) on the [_Spring and Autumn Annals_](https://en.wikipedia.org/wiki/Spring_and_Autumn_Annals)|[KR1e0007](https://github.com/kanripo/KR1e0007)*|21|
...
|老子|[_Laozi_](https://en.wikipedia.org/wiki/Tao_Te_Ching)|[KR5c0073](https://github.com/kanripo/KR5c0073)**|25|
|莊子|[_Zhuangzi_](https://en.wikipedia.org/wiki/Zhuangzi_(book))|[KR5c0126](https://github.com/kanripo/KR5c0126)|26-28|

*No standalone 正文 edition of this work exists in the Kanseki Repository (the catalog lists KR1d0025/KR1e0005 but they are repository-less stubs), so we use the SBCK commentary edition — 儀禮鄭注 (KR1d0026) and 春秋公羊經傳解詁 (KR1e0007) — and derive the bare classic as its main text layer. See [docs/corpus.md](docs/corpus.md).

**Lu Deming's 老子音義 keys to the 王弼本, not the SBCK 河上公章句; use the 正統道藏 (ZTDZ) branch of KR5c0073.

We omit chapter 1 of the _Jingdian Shiwen_, corresponding to the [_Erya_ (爾雅)](https://en.wikipedia.org/wiki/Erya). All digital sources have been preprocessed to remove punctuation, whitespace, and non-Chinese characters. Every source is Kanseki Repository data, generously licensed CC-BY.

Two constraints to observe when working with the sources (see [docs/corpus.md](docs/corpus.md) for details and per-work edition caveats): Kanripo repositories must be cloned from the branch recorded in [assets/docs.csv](assets/docs.csv), since the default branch is the HFL witness and can lack the commentary entirely; and annotations in `annotations.jsonl` are globally ordered by `meta.sequence` (equivalently by `(jdsw_id, index)` — sorting by `(juan_id, index)` interleaves lemma sequences across 卷 boundaries and silently breaks alignment).
